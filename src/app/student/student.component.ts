...
// Định nghĩa 1 mảng các phần tử user, gồm các thuộc tính:
  ...
  onChangeInput(event: any, key: string) {
    ...
    // spread operator
    this.newUser = {
      // tạo object mới bên trong có các cặp key value của pbject this.newuser
      ...this.newUser,
      // [key]: inputValue nếu key là năm tương đương với name: inputValue
      // với cú pháp spread ... thì name sẽ bị ghi đè để giá trị mới tư '' => inputValue
      [key]: inputValue
    };
  ...
  onSubmit() {
    ...
    if (!this.onValidate(this.newUser)) {
      // nếu ko pas qua validate thì sẽ retrun ra khỏi hàm submit luôn
      return;
    }
  ...
  onValidate(obj: any) {
    if (!obj.name || !obj.age || !obj.phone || !obj.img || !obj.email) {
      // tương tự if(obj.ten =='' || obj.cannang=='')
      return false;
    }
